...
drawModeBit(const kc85& kc, int chn_index, ubyte mask, const char* name, const char* on_str, const char* off_str) {
    ...
    if (kc.board->ctc.channels[chn_index].mode & mask) {
        ImGui::Text("%s", on_str);
    }
    else {
    ...
    if (ImGui::Begin(this->title.AsCStr(), &this->Visible, ImGuiWindowFlags_ShowBorders)) {
        ...
        for (int i = 0; i < z80ctc::num_channels; i++) {
            ...
            if (ImGui::CollapsingHeader(strBuilder.AsCStr())) {
                ImGui::Text("Time Constant: %02X", kc.board->ctc.channels[i].constant);
                ImGui::Text("Interrupt Vector: %02X", kc.board->ctc.channels[i].interrupt_vector);
                ImGui::Text("Mode Bits: %02X", kc.board->ctc.channels[i].mode);
